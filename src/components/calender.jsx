...
export default function Calender() {
  ...
  return (
    <div className=' max-h-screen overflow-y-auto sm:w-full md:w-1/4 bg-[white] rounded-3xl ml-10 mr-10' >     
      ...
      {event.filter((eventItem)=>{
        ...
        <div key={eventItem._id} className='flex font-Amaranth bg-primary text-white rounded-3xl m-5'>
          <p className='sm:text-2xl md:text-xl text-center text-primary m-3 p-2 bg-white rounded-xl '>{formatCalenderDate(new Date(eventItem.date))}</p>
          <h1 className='sm:text-3xl md:text-xl font-bold text-center p-5'>{eventItem.event}</h1>
          
        </div>
      ))}
      <div className='flex justify-center items-center pb-5'>
      <button className='bottom-0 sm:text-xl md:text-xl justify-items-center font-bold font-Amaranth text-[white] text-center w-1/2  bg-[red] rounded-3xl p-3'>View All events</button>
